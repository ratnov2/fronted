...
export const useVideo = () => {
	...
	const toggleVideo = useCallback((e?:MouseEvent<HTMLButtonElement>) => {
		e?.preventDefault()
		if (!isPlaying) {
	...
	const fastForward = (e?:MouseEvent<HTMLButtonElement>) => {
		e?.preventDefault()
		if (videoRef.current) videoRef.current.currentTime += 10
	}

	const revert = (e?:MouseEvent<HTMLButtonElement>) => {
		e?.preventDefault()
		if (videoRef.current) videoRef.current.currentTime -= 10
	}
